  ...
  private class ConnectionManager {
    ...
    SimpleServerRpcConnection register(SocketChannel channel) {
      ...
      if (LOG.isTraceEnabled()) {
        LOG.trace("Connection from " + connection + "; connections=" + size()
          + ", queued calls size (bytes)=" + callQueueSizeInBytes.sum() + ", general queued calls="
          + scheduler.getGeneralQueueLength() + ", priority queued calls="
          + scheduler.getPriorityQueueLength() + ", meta priority queued calls="
          + scheduler.getMetaPriorityQueueLength());
      }
      return connection;
    }
    ...
    synchronized void closeIdle(boolean scanAll) {
      ...
      for (SimpleServerRpcConnection connection : connections) {
        ...
        if (
          connection.isIdle() && connection.getLastContact() < minLastContact && close(connection)
